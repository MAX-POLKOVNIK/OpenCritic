...
@Composable
fun RatingReviewsScreen(
    args: RatingReviewsRoute.InitArgs,
    navController: NavController,
    viewModel: RatingReviewsViewModel = koinViewModel { parametersOf(args) }
) {
    ...
    Scaffold(
        ...
        when (val s = state) {
            is RatingReviewsState.Content-> RatingReviewsStateContent(s, Modifier.padding(paddings))
            is RatingReviewsState.Loading -> LoadingBox(s, Modifier.padding(paddings))
            is RatingReviewsState.Error -> ErrorBox(s, Modifier.padding(paddings))
        }
    }
}