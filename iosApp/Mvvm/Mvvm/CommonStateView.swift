...
public struct CommonStateView<Content: AnyObject, ContentView>: View where ContentView: View {
    let state: CommonViewModelState<Content>
    
    @ViewBuilder 
    let viewProducer: (Content) -> ContentView
    
    public init(state: CommonViewModelState<Content>, viewProducer: @escaping (Content) -> ContentView) {
        self.state = state
